...
public static class ByteExtension
{
    ...
    {
        return haystack.IndexOf(needle, start: 0);
    }
    ...
    {
        return haystack.IndexOf(needles, start: 0);
    }
    ...
    public static bool Match(this Memory<byte> haystack, ReadOnlyMemory<byte> needle, int start)
    {
        ...
        for (var i = 0; i < needle.Length; i++)
        {
            if (needle.Span[i] != haystack.Span[index: i + start])
            {
                return false;
            }
        }
    ...
    public static bool Match(this Memory<byte> haystack, byte[] needle, int start)
    {
        ...
        for (var i = 0; i < needle.Length; i++)
        {
            if (needle[i] != haystack.Span[index: i + start])
            {
                return false;
            }
        }
    ...
    public static bool Match(this Span<byte> haystack, byte[] needle, int start)
    {
        ...
        for (var i = 0; i < needle.Length; i++)
        {
            if (needle[i] != haystack[index: i + start])
            {
                return false;
            }
        }
