...
public static class TypeInformationExtension
{
    public static bool IsEnumerable(this TypeInformation type)
    {
        ...
        if (type.Type.GetGenericTypeDefinition() != typeof(IEnumerable<>)) return false;
    ...
    {
        if (type.Constructors.Length == 0) return null;

        if (type.Constructors.Length > 1) throw new ToManyConstructorsException(type.Type, 1, type.Constructors.Length);
